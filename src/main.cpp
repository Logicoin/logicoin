...
{
    // LogiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, logicoin - DarkGravity v2, written by Evan Duffield - evan@logicoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LogiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LogiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000b9347003935916f263ae6d45ef6e64b173ad2eff5fe16e26a94dff5536a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397090872;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 592149;

        if (fTestNet)
        {
            block.nTime    = 1397088621;
            block.nNonce   = 97207;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x26ea5e843b92d13893e38deb922adfa2f29193927e4c43ed88f61f8a425f49d6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // LogiCoin: increase each by adding 2 to bitcoin's value.
...
void static LogiCoinMiner(CWallet *pwallet)
{
    printf("LogiCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("logicoin-miner");
    ...
    try { loop {
        ...
        printf("Running LogiCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LogiCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LogiCoinMiner, pwallet));
}
